...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TokioStream.swiftmodule/`
...
//      `import TokioStream` above would fail at compile time.
//
//   2. The static archive `libTokioStream.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TokioStream" }` and
//      `flattenPackage = "io.github.kotlinmania.tokiostream"` configuration in
...
//      via the `TokioStreamLibrary` product.
//
...
final class TokioStreamExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TokioStream swift module imported cleanly")
    }
}
