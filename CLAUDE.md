# Claude Code Project Instructions — tokio-stream-kotlin

## Project Overview

This is **tokio-stream-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tokio-stream`](https://crates.io/crates/tokio-stream).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tokio-stream/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tokio-stream>
